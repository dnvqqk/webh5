...
var WmyTar = /** @class */ (function () {
    ...
    WmyTar.prototype.init = function () {
         ...
         */
        if (TAR.ENV.ANDROID) {
            this._render.on('TAR_STATE_CHANGE', function () {
                var vrDisplay = _this_1._ar.getVRDisplay();
                if (vrDisplay && _this_1._ar.isEngineDownload()) {
                    var frameData = new window["VRFrameData"]();
                    vrDisplay.getFrameData(frameData);
                    var _a = frameData.pose.position, x = _a[0], y = _a[1], z = _a[2];
                    if (x === 0 && y === 0 && z === 0) {
                        _this_1._ar.onTarStateChanged('limited');
                    }
                    else {
                        _this_1._ar.onTarStateChanged('normal');
                    }
                }
            });
        }
        // trigger camera position change every frame
        this._render.on('CAMERA_TRANSFORM_CHANGE', function () {
            var vrDisplay = _this_1._ar.getVRDisplay();
            // 需要获取到vrDisplay对象并且ar引擎下完成才能做业务逻辑
            if (vrDisplay && _this_1._ar.isEngineDownload()) {
                _this_1._engine.onCameraTransformChange();
            }
        });
        // render初始化，运行主循环
        this._render.init();
